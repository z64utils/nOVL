...
static long mr4kd_conf = 
    MR4KD_SPACING(16) | MR4KD_RLOWER | MR4KD_OLOWER | MR4KD_HLOWER;
...
static int mr4kd_rcpy_gpr ( char * dest, int reg )
{
    ...
    /* Color? */
    if( MR4KD_FLAG_GET(MR4KD_COLOR) )
    {
        i += sprintf( &dest[i], "%s", ANSI_SET_FG_BLUE );
    }
    ...
    for( ; registers_a_gpr[reg][s]; i++, s++ )
        dest[i] = (char)( (mr4kd_conf & MR4KD_RLOWER) ? tolower((uint8_t)registers_a_gpr[reg][s]) : (uint8_t)registers_a_gpr[reg][s] );
    
    /* Color? */
    if( MR4KD_FLAG_GET(MR4KD_COLOR) )
    {
        i += sprintf( &dest[i], "%s", ANSI_RESET_DEFAULT );
    }
...
static int mr4kd_rcpy_fpr ( char *dest, int reg )
{
    ...
    for(; registers_a_fpr[reg][s]; i++, s++ )
        dest[i] = ( (mr4kd_conf & MR4KD_RLOWER) ? tolower((uint8_t)registers_a_fpr[reg][s]) : (uint8_t)registers_a_fpr[reg][s] );
    dest[i] = 0;
